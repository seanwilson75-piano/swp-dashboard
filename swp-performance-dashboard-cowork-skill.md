...
*Version: 6.5*
*Last updated: 2026-06-21 — Fixed false 1029/1045 spike alerts by switching the sales-spike comparison from `byProduct` (YTD cumulative) vs an often-empty/stale SureCart-sourced `SC_PREV_30` to `byProductPeriod.monthly` (current-month-to-date) vs `SC_PREV_30` sourced from Airtable's prior full calendar month (Step 2B) — comparing a YTD total against a single 30-day baseline was producing nonsensical "500%+ this month" alerts. `SC_PREV_30 = {}` silently suppresses sales alerts rather than firing false positives. (Note: an earlier draft of this entry described renaming `SC_PREV_30` to `SC_PREV_WEEK` for a week-over-week comparison — that was never implemented in the dashboard JS and `SC_PREV_30`/`.monthly` is what's actually live; this entry has been corrected to match.) Dashboard HTML updated: Sales Detail KPI grid no longer duplicates Pageviews/Unique Visitors already shown in the global strip; "New Signups" renamed to "New Members" throughout; recurring subscription rows with 0 views and real sales now show an auto-renew explanatory note.*
